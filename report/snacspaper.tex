...
%In case the condition is satisfied we follow the left branch of the node in order to nd a so-called child node, that has a dierent test condition. In case the condition "Is age > 50?" is not satised, we follow the right branch and we nd a dierent child node with, again, a dierent test condition. This process continues until we nd the terminal nodes that contain the classes in which our observations are classied
